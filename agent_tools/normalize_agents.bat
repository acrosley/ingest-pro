...
REM Check if database exists in parent directory
if not exist "..\database_tools\call_recordings.db" (
    echo ERROR: Database file not found: ..\call_recordings.db
    echo.
    echo Please ensure the database file exists in the parent directory.
    echo.
    pause
    exit /b 1
)
