...
try:
    ...
    GEMINI_STT_MODEL_NAME = config.get('Transcription', 'GeminiModelName').strip()
...
def process_transcription_with_gemini(path):
    if not api_key_configured: return
    logger.info(f"[STT] Starting transcription for: {path}")
    uploaded_file_info = None
    try:
        display_name = f"call-rec-{os.path.basename(path)}-{int(time.time())}"
        uploaded_file_info = genai.upload_file(path=path, display_name=display_name)
        if uploaded_file_info and getattr(uploaded_file_info, 'name', None):
            wait_for_file_activation(uploaded_file_info.name, max_wait_seconds=180)
        noun_guidance = ""
        if EXPECTED_NOUNS_LIST:
            noun_guidance = "Pay special attention to accurately transcribing: " + ", ".join(f"'{n}'" for n in EXPECTED_NOUNS_LIST) + "."
        system_instr = ("You are an expert transcription assistant. Accurately transcribe this audio, "
                        "label speakers as 'Agent:' and 'Caller:'. Include timestamps [HH:MM:SS]. " + noun_guidance)
        user_prompt = "Please transcribe the uploaded call audio."
        model = genai.GenerativeModel(model_name=GEMINI_STT_MODEL_NAME, system_instruction=system_instr)
        response = model.generate_content([user_prompt, uploaded_file_info], request_options={"timeout": GEMINI_STT_TIMEOUT})
        transcript = extract_text_from_gemini_response(response)
        if not transcript.strip():
            # Retry once more in case of race/lag
            time.sleep(1.5)
            response = model.generate_content([user_prompt, uploaded_file_info], request_options={"timeout": GEMINI_STT_TIMEOUT})
            transcript = extract_text_from_gemini_response(response)
        audio_path = Path(path)
        user_base_dir = audio_path.parent.parent
        # Always write the user-local transcript (for existing pipeline expectations)
        ...
        if transcript.strip():
            ...
            try:
                ...
                # Write a fresh copy to ensure atomic content
        ...
        if uploaded_file_info:
            try:
                ...
                logger.error(f"[STT] Failed to delete '{uploaded_file_info.name}': {e_del}")
...
def transcription_worker():
    while not shutdown_requested.is_set():
        ...
        try:
            ...
            try:
                process_transcription_with_gemini(path)
...
# ----------------------------------------------------------------------
try:
    from transcript_tools.transcript_normalizer_module import transcript_normalizer
...
# ----------------------------------------------------------------------
try:
    from database_tools.archive_and_normalize import ArchiveAndNormalizer
